...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      # menuItem("", tabName = "map_weather", icon = icon("")),
      selectInput(
        "location",
        "Choose Country:",
  ...
  dashboardBody(
    ...
    box(
      ...
      width = 12,
      height = NULL,
      "Click a mountain on the map to display information and a weather forecast (it may take a minute)\nYou can filter based on Country, State, and Grade using the sidebar on the left"
    )
  ), # fluidRow(
  # tabItem(tabName = "map_weather",
  fluidRow(
    column(width = 6, box(
...
server <- function(input, output) {
  ...
  observeEvent(input$leafletMap_marker_click, {
    ...
    output$weather_info <- renderPlot({
      ggplot(weatherdata_long, aes(date, Values, colour = Weather)) +
        geom_line() +
