...
[The World's Hardest Boulder Problems Sent by Women](#the-world\'s-hardest-boulder-problems-sent-by-women)<br>
[V15 Club](#V15-Club)<br>
[Who Is the Strongest Female Boulderer Ever?](#who-is-the-strongest-female-boulderer-ever\?)<br>
[Trends Over Time](#trends-over-time)<br>
[How Many Women Can Climb V13 or Higher?](#how-many-women-can-climb-v15-or-higher\?)<br>
...
# The World's Hardest Boulder Problems Sent by Women

```{r, echo=FALSE}
#mydata2 <- mydata %>% group_by(Route) %>% mutate(count = n()) %>% mutate(repeated = case_when (count == 1 ~ "Unrepeated", count > 1 ~ "Repeated"))
DT::datatable(mydata[,c(1:8)], colnames = c('Route Name', 'Climber', 'Consensus VGrade', 'FA', 'Country', 'Area', 'Date', 'Suggested VGrade by Climber'), options = list(pageLength = 900, autoWidth = TRUE))
```
# V15 Club
...
# Who Is the Strongest Female Boulderer Ever?
...
# How Many Women Can Climb V13 or Higher?
...
# Which Countries Have the Hardest Boulder Problems for Women?
